...
const HIGHLIGHT_COLOR_CODE := Color("fafa82")
...
func get_levels_bbcode(unit_count: int) -> String:
	...
	for i: int in levels:
		if i == (reached_levels.size()-1):
			code.append("[color=#%s]%s[/color]" % [HIGHLIGHT_COLOR_CODE.to_html(false), unit_requirements[i]])
		else:
...
func _get_reached_levels(unit_count: int) -> Array[int]:
